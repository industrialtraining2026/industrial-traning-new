...
class DocumentIndexer:
    def __init__(self):
        ...
        # Try Groq first (if available), then Google AI, then OpenAI, then local
        # Groq uses OpenAI-compatible API, so we can use it for embeddings too
        if (settings.GROQ_API_KEY and 
            settings.GROQ_API_KEY.strip() and 
            settings.GROQ_API_KEY != "PUT_YOUR_GROQ_API_KEY_HERE" and
            settings.GROQ_API_KEY != "your_groq_api_key_here"):
            # Groq may support OpenAI-compatible embeddings, try OpenAI client with Groq endpoint
            self.embedder = EmbeddingGenerator(
                api_key=settings.GROQ_API_KEY,
                use_local=False,
                use_groq=True  # Pass Groq flag
            )
        elif (settings.GOOGLE_API_KEY and 
