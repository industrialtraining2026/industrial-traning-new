...
class EmbeddingGenerator:
    def __init__(self, api_key: str = None, use_local: bool = False, use_google: bool = False, use_groq: bool = False):
                ...
                if api_key:
                    try:
                        if OpenAI is not None:
                            ...
                            logger.warning("OpenAI client not available, using module-level API")
            ...
            if api_key:
                try:
                    if OpenAI is not None:
                        ...
                        logger.warning("OpenAI client not available for Groq, using module-level API")
                        self.openai_client = None
                except Exception as e:
            ...
            if api_key and api_key.strip() and api_key != "your_google_gemini_api_key_here":
                try:
                    ...
                    if SENTENCE_TRANSFORMERS_AVAILABLE:
                        ...
                        if openai_key:
                            ...
                            # No OpenAI key available, will fallback to hashing embeddings
                            logger.warning("No OpenAI API key available, will use hashing embeddings fallback")
                    ...
                    if SENTENCE_TRANSFORMERS_AVAILABLE:
                        try:
                            ...
                            logger.error(f"Failed to load sentence-transformers: {str(e2)}, will use hashing fallback")
                            self.use_google = False
                            self.use_local = False
                            self.api_key = None
                    else:
                        logger.warning("No embedding solution available, will use hashing embeddings fallback")
                        self.use_google = False
                        self.use_local = False
                        self.api_key = None
            else:
                ...
                if SENTENCE_TRANSFORMERS_AVAILABLE:
                    try:
                        ...
                        logger.error(f"Failed to load sentence-transformers: {str(e)}, will use hashing fallback")
                        self.use_local = False
                        self.use_google = False
                        self.api_key = None
                else:
        ...
        # Check if we have no valid embedding method available
        has_no_valid_embedding = (
            (not self.use_local) and 
            (not self.use_google) and 
            (not self.use_groq) and 
            (not self.api_key) and 
            (not self.openai_client)
        )
        
        if (not getattr(self, "embedding_dim", None)) or has_no_valid_embedding:
            self._enable_hashing_fallback()
        elif (not SENTENCE_TRANSFORMERS_AVAILABLE) and self.use_local:
            self._enable_hashing_fallback()
        elif (not SENTENCE_TRANSFORMERS_AVAILABLE) and (not self.api_key) and (not self.openai_client) and (not self.use_groq) and (not self.use_google):
            self._enable_hashing_fallback()
