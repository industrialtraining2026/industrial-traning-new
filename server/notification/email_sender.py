...
class EmailSender:
    ...
    def __init__(self):
        ...
        # ✅ IMPORTANT: use Brevo-safe sender email
        self.from_email = os.getenv(
            "FROM_EMAIL",
            "no-reply@brevosend.com"
        ).strip()
    ...
    def _send_single_email(
        ...
        # ✅ Log messageId for verification
        try:
            resp_json = response.json()
            logger.info(
                f"✅ Brevo email sent → {to_email}, messageId={resp_json.get('messageId')}"
            )
        except Exception:
            logger.info(f"✅ Brevo email sent → {to_email}")
