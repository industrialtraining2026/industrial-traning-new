...
class EmailSender:
	...
	self.email_provider = os.getenv("EMAIL_PROVIDER", "smtp")
        ...
        logger.info(f"Email Sender initialized:")
    ...
    def send_notification(
        ...
        # Add summary message
        if results["failed"] > 0:
            results["error"] = f"Failed to send {results['failed']} out of {results['total_recipients']} emails. Check errors for details."
        elif results["sent"] == 0:
            results["error"] = "No emails were sent. Please check configuration."
        
        return results
    
    def _send_single_email(self, to_email: str, subject: str, html_body: str, text_body: str):
    """Send a single email (SMTP or SendGrid)"""
    ...
    if self.email_provider == "sendgrid":
        ...
        try:
            ...
            error_msg = f"SMTP authentication failed. Please check your SMTP_USERNAME and SMTP_PASSWORD. If using Gmail, make sure you're using an App Password (not your regular password). Error: {str(e)}"
            logger.error(error_msg)
            raise ValueError(error_msg)
        except smtplib.SMTPConnectError as e:
            error_msg = f"Failed to connect to SMTP server {self.smtp_host}:{self.smtp_port}. Please check your SMTP_HOST and SMTP_PORT. Error: {str(e)}"
            logger.error(error_msg)
            raise ValueError(error_msg)
        except smtplib.SMTPServerDisconnected as e:
            error_msg = f"SMTP server disconnected. Server: {self.smtp_host}:{self.smtp_port}. Error: {str(e)}"
            logger.error(error_msg)
            raise ValueError(error_msg)
            ...
            error_msg = f"Connection timeout. Could not connect to {self.smtp_host}:{self.smtp_port} within 30 seconds."
            logger.error(error_msg)
            raise ValueError(error_msg)
        except Exception as e:
    ...
    def _send_via_smtp(self, to_email, subject, html_body, text_body):
    	if not self.smtp_username or not self.smtp_password:
        	raise ValueError(
            		"SMTP credentials not configured. "
            		"Please set SMTP_USERNAME and SMTP_PASSWORD."
        	)
