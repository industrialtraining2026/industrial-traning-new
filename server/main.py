...
def list_pdfs(pdf_type: str):
    ...
    try:
        ...
        target_dir = pdf_manager.get_directory(pdf_type)
        logger.info(f"Listing PDFs for {pdf_type} from directory: {target_dir}")
        logger.info(f"Directory exists: {target_dir.exists()}")
        if target_dir.exists():
            all_files = list(target_dir.glob("*"))
            logger.info(f"All files in directory: {[f.name for f in all_files]}")
        
        pdf_list = pdf_manager.list_pdfs(pdf_type)
        logger.info(f"Found {len(pdf_list)} PDF files for {pdf_type}")
        ...
        return {
            ...
            "debug": {
                ...
                "directory_exists": target_dir.exists() if target_dir else False
            }
        }
    except Exception as e:
