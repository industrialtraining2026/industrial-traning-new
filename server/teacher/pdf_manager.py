    ...
    def upload_pdf(self, file_content: bytes, filename: str, pdf_type: str, uploaded_by: str = "teacher") -> Dict[str, Any]:
        ...
        try:
            ...
            if not file_path.exists():
                raise IOError(f"File was not written successfully to {file_path}")
            
            # Get file info
            file_size = os.path.getsize(file_path)
            upload_time = datetime.now().isoformat()
            
            logger.info(f"Uploaded PDF: {safe_filename} ({file_size} bytes) to {pdf_type} directory at {file_path}")
